 ...
 * 6 square feet of slack, for a total of 58 square feet. A present with dimensions 1x1x10 requires
 * 2*1 + 2*10 + 2*10 = 42 square feet of wrapping paper plus 1 square foot of slack, for a total of
 * 43 square feet.
 * <p>
...
public class Day02 {
  ...
  public static void main(String[] args) throws IOException {
    ...
    try (BufferedReader br = new BufferedReader(new FileReader("inputs/2015/day2.txt"))) {
      String dimensions;
      while ((dimensions = br.readLine()) != null) {
        totalSquareFeet += squareFeet(dimensions);
        totalRibbonToOrder += totalFeetOfRibbon(dimensions);

      }
    }
    System.out.println(totalSquareFeet);
    System.out.println(totalRibbonToOrder);
  }

  public static int squareFeet(String dimensions) {
   ...
   */
  public static int totalFeetOfRibbon(String dimensions) {
    String[] dimension = dimensions.split("x");
    int length = Integer.parseInt(dimension[0]);
    int width = Integer.parseInt(dimension[1]);
    int height = Integer.parseInt(dimension[2]);
    int perimeter = 2 * ((length + width + height) - Math.max(length, Math.max(width, height)));
